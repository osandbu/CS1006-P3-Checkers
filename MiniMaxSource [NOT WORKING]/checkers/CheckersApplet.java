...
public class CheckersApplet extends JApplet implements MouseMotionListener,
	...
	public void doAI() {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				boolean miniMax = false;// TODO
				if (miniMax) {
					alphaBetaMinimax();
					if (bestMove == null)
						gameOver();
					else
						board.animateMove(bestMove);
				} else {
	...
	public int alphaBetaMinimax() {
		...
		bestMove = null;
		return alphaBetaMinimax(board, alpha, beta, 0);
	}
	 ...
	 * @return
	 */
	public int alphaBetaMinimax(GamePosition node, int alpha, int beta,
			int depth) {
		if (depth == Constants.DEPTH_LIMIT) {
			return node.value();
		}
		if (node.isGameOver()) {
			return node.value(board.getCurrentPlayer());
		}
		// Get all possible captures, and if there are none, get all moves.
		ArrayList<Move> possibleMoves = node.getAllValidMoves();
		if (depth == ROOT) {
			/*
			 * PossibleMoves contains at least one element, because the game is
			 * not over.
			 */
		...
		if (board.getCurrentPlayer() == node.getCurrentPlayer()) {
			for (Move move : possibleMoves) {
				GamePosition child = node.applyMove(move);
				int result = alphaBetaMinimax(child, alpha, beta, depth++);
				if (result > alpha) {
					alpha = result;
					if (depth == ROOT)
						bestMove = move;
